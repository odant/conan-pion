...
const std::string   types::REQUEST_METHOD_OPTIONS("OPTIONS");
...
{
    static std::locale _s_locale(std::locale("English"), new boost::posix_time::time_facet("%a, %d %b %Y %H:%M:%S GMT"));
    
    std::ostringstream stream;
    stream.imbue(_s_locale);
    stream << boost::posix_time::from_time_t(t);
    return stream.str();
}
